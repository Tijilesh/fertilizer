...
const EditProduct = ({ onSuccess }) => {
  ...
  const fetchProduct = async () => {
    try {
      ...
      alert(t('errorLoadingProduct') || 'Error loading product')
      navigate('/products')
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || t('errorUpdatingProduct') || 'Error updating product')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="glass-card premium-shadow p-10 rounded-3xl border border-white/20">
        <h1 className="text-3xl font-bold text-gray-900 mb-8 tracking-tight">{t('editProduct')}</h1>
