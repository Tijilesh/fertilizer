...
import { MessageSquare, Star, Send, CheckCircle, Users, TrendingUp, ShoppingBag, Package, Info, Search, ShoppingCart, CreditCard, PiggyBank, Heart, Calculator, AlertTriangle, BookOpen, Cloud, MessageCircle, Building, BarChart3, FileText, Share } from 'lucide-react'
...
const Survey = () => {
  ...
  const ratingOptions = [
    { value: '1', label: t('ratingPoor') || 'Poor', color: 'text-red-600' },
    { value: '2', label: t('ratingFair') || 'Fair', color: 'text-orange-600' },
    { value: '3', label: t('ratingGood') || 'Good', color: 'text-yellow-600' },
    { value: '4', label: t('ratingVeryGood') || 'Very Good', color: 'text-green-600' },
    { value: '5', label: t('ratingExcellent') || 'Excellent', color: 'text-emerald-600' }
  ]
  ...
  const handleSubmit = (e) => {
    ...
    if (!hasRatings) {
      toast.error(t('surveyValidationError') || 'Please provide at least some feedback before submitting.')
      return
    }
    ...
    toast.success(t('surveySuccessToast') || 'Thank you for your valuable feedback!')
    setSubmitted(true)
      ...
      <div className="flex space-x-2">
        {ratingOptions.map((option) => (
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-xl border-2 transition-all duration-200 ${responses[question] === option.value
  ...
  const TextQuestion = ({ question, label, placeholder, rows = 3, responses, handleTextChange }) => (
    ...
    return (
      <div className="flex items-center justify-center min-h-[80vh]">
        <div className="text-center space-y-6">
          ...
          <div>
            <h2 className="text-3xl font-bold text-slate-800 mb-2">{t('thankYou')}</h2>
            <p className="text-lg text-slate-600">{t('feedbackSubmittedSuccess')}</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      {/* Header */}
      <div className="text-center glass-card premium-shadow p-12 rounded-3xl border border-white/20">
        <div className="w-16 h-16 mx-auto bg-emerald-100 rounded-2xl flex items-center justify-center mb-6">
          <MessageSquare className="w-8 h-8 text-emerald-600" />
        </div>
        <h1 className="text-gradient text-4xl font-extrabold mb-4">{t('helpImproveAgriFert')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            <Users className="w-6 h-6 text-emerald-600" />
            <h2 className="text-2xl font-bold text-slate-800">{t('userExperience')}</h2>
          </div>
          <div className="space-y-8">
            ...
            <RatingQuestion
              ...
              label={t('qInterfaceDesign') || "How would you rate the overall interface design?"}
              icon={Star}
            />
            <RatingQuestion
              question="mobileExperience"
              label={t('qMobileExperience') || "How well does the application work on mobile devices?"}
              icon={CheckCircle}
            />
          </div>
        </div>
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('productFeatures')}</h2>
          </div>
          <div className="space-y-8">
            <RatingQuestion
              question="productVariety"
              label={t('qProductVariety') || "How satisfied are you with the variety of products available?"}
              icon={Package}
            />
            <RatingQuestion
              question="productInfo"
              label={t('qProductInfo') || "How comprehensive and helpful is the product information?"}
              icon={Info}
            />
            <RatingQuestion
              question="searchFilters"
              label={t('qSearchFilters') || "How effective are the search and filter options?"}
              icon={Search}
            />
          </div>
        </div>
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('shoppingFeatures')}</h2>
          </div>
          <div className="space-y-8">
            <RatingQuestion
              question="cartFunctionality"
              label={t('qCartFunctionality') || "How well does the shopping cart functionality work?"}
              icon={ShoppingCart}
            />
            <RatingQuestion
              question="checkoutProcess"
              label={t('qCheckoutProcess') || "How smooth is the checkout and ordering process?"}
              icon={CreditCard}
            />
            <RatingQuestion
              question="paymentOptions"
              label={t('qPaymentOptions') || "How satisfied are you with the available payment options?"}
              icon={PiggyBank}
            />
          </div>
        </div>
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('additionalFeatures')}</h2>
          </div>
          <div className="space-y-8">
            <RatingQuestion
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('ownerManagementFeatures')}</h2>
            <span className="text-sm text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{t('forOwnersManagers') || 'For shop owners and managers'}</span>
          </div>
          <div className="space-y-8">
            <RatingQuestion
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('overallSatisfaction')}</h2>
          </div>
          <div className="space-y-8">
            <RatingQuestion
        ...
        <div className="glass-card premium-shadow p-8 rounded-3xl border border-white/20 transition-all duration-300">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800">{t('additionalFeedback')}</h2>
          </div>
          <div className="space-y-8">
            <TextQuestion
